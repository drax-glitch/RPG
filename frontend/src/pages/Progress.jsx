...
export default function ProgressPage() {
  ...
  useEffect(() => {
    api.fetchProgress().then(setData);
  }, []);

  if (!data) return <p className="text-slate-500">Loading progress…</p>;

  const summary = [
    { icon: <Zap size={16} className="text-arcane-400" />, value: data.totalXp.toLocaleString(), label: "Total XP" },
    { icon: <CheckSquare size={16} className="text-emerald-400" />, value: data.questsDone, label: "Quests Done" },
    { icon: <Flame size={16} className="text-orange-400" />, value: `${data.bestStreak}d`, label: "Best Streak" },
    { icon: <Coins size={16} className="text-amber-400" />, value: data.goldEarned.toLocaleString(), label: "Gold Earned" },
  ];

  return (
    <div className="space-y-6">
      <h1 className="font-display text-2xl text-white flex items-center gap-2">📊 Progress Report</h1>
